...
namespace CAT.Message.Internals
{
    ...
    {
        private readonly Action<ITransaction, string> _endCallBack;
        private IList<IMessage> _mChildren;
        private long _mDurationInMicro; // must be less than 0

        public DefaultTransaction(String type, String name, Action<ITransaction, string> endCallBack)
            : base(type, name)
        {
            _mDurationInMicro = -1;
        ...
        public override void Complete(string messageTreeId)
        {
            if (IsCompleted())
            {
            ...
            else
            {
                ...
                {
                    _endCallBack(this, messageTreeId);
                }
            }
        }
